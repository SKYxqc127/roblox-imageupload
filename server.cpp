...
bool downloadImage(const std::string& url, const std::string& path) {
    ...
    if (!curl) { std::cerr << "❌ curl init failed" << std::endl; return false; }

    std::ofstream ofs(path, std::ios::binary);
    if (!ofs.is_open()) { std::cerr << "❌ cannot open file for writing: " << path << std::endl; curl_easy_cleanup(curl); return false; }
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0 Safari/537.36");
...
ImageData loadImageToMatrix(const std::string& url, int resize) {
    if (!(url.rfind("https://cdn.discordapp.com/",0)==0 || url.rfind("https://media.discordapp.net/",0)==0)) throw std::runtime_error("Only Discord image links allowed");
    ...
    if(test.good()) { std::cout << "📀 Cache hit\n"; test.close(); tmp=cachePath; }
        ...
        std::cout << "⬇ Downloading new image...\n"; 
        if(!downloadImage(fixedUrl, cachePath)) throw std::runtime_error("download failed"); 
        tmp = cachePath; 
    }
...
std::string toJson(const ImageData& img) {
    ...
    for(int y=0;y<img.height;++y){ s<<"["; for(int x=0;x<img.width;++x){ const auto &p=img.pixels[y][x]; s<<"["<<(int)p[0]<<","<<(int)p[1]<<","<<(int)p[2]<<"]"; if(x<img.width-1) s<<","; } s<<"]"; if(y<img.height-1)s<<","; }
    s<<"]}"; return s.str();
}
...
int main() {
    ...
    int port=8787;
    int sockfd = socket(AF_INET,SOCK_STREAM,0); if(sockfd<0) { std::cerr<<"socket failed\n"; return 1; }
    ...
    while(true){
        ...
        if(req.rfind("OPTIONS ",0)==0){
            ...
            ssize_t _=write(client,res.c_str(),res.size());(void)_; close(client); continue;
        }

        try{
            Query q=parseQuery(req);
            if(q.imageUrl.empty()){ std::string body="{\"message\":\"Use /?url=<encoded URL>&resize=32\"}"; std::string res=httpOkJson(body); ssize_t _=write(client,res.c_str(),res.size()); (void)_; close(client); continue;}
            auto img=loadImageToMatrix(q.imageUrl,q.resize);
            std::string body=toJson(img); std::string res=httpOkJson(body); ssize_t _=write(client,res.c_str(),res.size()); (void)_;
        } catch(const std::exception& e){
            std::string res=httpErrJson(500,e.what()); ssize_t _=write(client,res.c_str(),res.size()); (void)_;
        }
        close(client);
    }
    return 0;
